...
@patch("s4.clarity.scripts.stepepp.LIMS")
class TestStepEPP(TestCase):
    def test_prefetch(self, mock_lims):
        ...
        self._execute_prefetch_tests(inputs, outputs, samples, mock_lims)
    ...
    def _execute_prefetch_tests(self, inputs, outputs, samples, mock_lims):
        stepepp = SomeStepEPP(FakeOptions)
        stepepp.lims = mock_lims
